...
def start(video_sample):
    ...
    # Read model
    face_recognizer.read(curr_dir+'/EigenFacesModel.xml')
    cap = cv2.VideoCapture(f'samples/{video_sample}-sample.mp4')
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            #     image_paths[result[0]]), (x, y-25), 2, 1.1, (0, 255, 0), 1, cv2.LINE_AA)
            ...
            if result[1] < 8000:
                ...
                cv2.putText(frame, 'Unknown', (x, y-25), 2,
                            1.1, (0, 0, 255), 1, cv2.LINE_AA)
                cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
